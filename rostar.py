...
/* 2. カラムを強制的に1/3幅にし、折り返しや横はみ出しを禁止 */
...
selected_team = st.sidebar.selectbox("球団", available_teams)
                ...
                for col_idx, (_, player) in enumerate(row_players.iterrows()):
                    ...
                    with cols[col_idx]:
                        ...
                        <div class="roster-card">
                            <div class="roster-name">#{player['背番号']} {p_name}{badge}</div>
                            <div class="roster-sub">{p_age}歳 / {player['投打']}</div>
                        </div>
                        """, unsafe_allow_html=True)

                        # タップするたびに状態が切り替わるボタン（プルダウン幅の制約を回避）
                        btn_label = STATUS_DISPLAY.get(cur_stat, cur_stat)
                        if st.button(btn_label, key=f"btn_{selected_team}_{p_no}"):
                            # 次のステータスへ循環切り替え
    ...
    for row_idx in range(0, len(ikusei_df), 3):
        ...
        for col_idx, (_, player) in enumerate(row_players.iterrows()):
            ...
            with ikusei_cols[col_idx]:
                st.markdown(f"<div class='roster-card'><div class='roster-name'>#{player['背番号']} {p_name}</div><div class='roster-sub'>{player['守備位置']} {p_age}歳</div></div>", unsafe_allow_html=True)
...
with res2:
    if remaining_slots >= 0:
        st.markdown(f"**70枠まで: あと <span style='color:#16a34a; font-size:1.1rem; font-weight:bold;'>{remaining_slots}</span> 枠**", unsafe_allow_html=True)
    else:
        st.markdown(f"**超過: <span style='color:#dc2626; font-size:1.1rem; font-weight:bold;'>{-remaining_slots}</span> 人**", unsafe_allow_html=True)
